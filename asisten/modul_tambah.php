    ...
    if (empty($judul) || $praktikum_id <= 0 || $pertemuan <= 0) {
        ...
        if (isset($_FILES['file_materi']) && $_FILES['file_materi']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($ext, $allowed)) {
                $form_message = "Format file tidak valid. Hanya PDF, DOC, atau DOCX.";
                $form_message_type = "error";
            } else {
                $upload_dir = '../db/materi/';
                if (!is_dir($upload_dir)) mkdir($upload_dir, 0777, true);
                $file_materi_name = 'materi_' . time() . '_' . basename($_FILES['file_materi']['name']);
                if (move_uploaded_file($_FILES['file_materi']['tmp_name'], $upload_dir . $file_materi_name)) {
                    $file_materi = $file_materi_name;
                } else {
                    $form_message = "Gagal mengunggah file.";
                    $form_message_type = "error";
                }
            }
        }
        if (empty($form_message)) {
            $stmt = $conn->prepare("INSERT INTO modul (praktikum_id, judul, file_materi, pertemuan_ke) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("issi", $praktikum_id, $judul, $file_materi, $pertemuan);
            if ($stmt->execute()) {
                $_SESSION['last_action_message'] = 'Modul "' . htmlspecialchars($judul) . '" berhasil ditambahkan!';
                header("Location: modul.php");
                exit();
            } else {
                $form_message = "Gagal menyimpan data modul.";
                $form_message_type = "error";
            }
            $stmt->close();
        }
...
<body class="min-h-screen flex items-center justify-center p-4">
    <div class="bg-[#1E293B] p-8 rounded-xl shadow-2xl w-full max-w-lg border border-slate-700">
        <div class="text-center mb-8">
            ...
            <p class="text-slate-400 mt-2">Buat modul baru untuk praktikum.</p>
        </div>
        ...
        <form method="post" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="judul" id="judul" required class="mt-1 block w-full px-4 py-3 bg-slate-700/50 border border-slate-600 rounded-md shadow-sm placeholder-slate-400 text-white focus:outline-none focus:ring-2 focus:ring-purple-500 sm:text-sm" placeholder="Contoh: Pengenalan OOP">
            </div>
            <div>
                <label for="pertemuan_ke" class="block text-sm font-medium text-slate-300 mb-1">Pertemuan Ke-</label>
                <input type="number" name="pertemuan_ke" id="pertemuan_ke" min="1" required class="mt-1 block w-full px-4 py-3 bg-slate-700/50 border border-slate-600 rounded-md shadow-sm placeholder-slate-400 text-white focus:outline-none focus:ring-2 focus:ring-purple-500 sm:text-sm" placeholder="Contoh: 1">
            </div>
            <div>
                <label for="file_materi" class="block text-sm font-medium text-slate-300 mb-1">Upload File Materi</label>
                ...
                <p class="mt-1 text-xs text-slate-500">Tipe file: PDF, DOC, DOCX. Ukuran maks: 2MB.</p>
            </div>
            <div class="flex justify-between items-center pt-4">
                ...
                <button type="submit" class="inline-flex items-center px-6 py-2.5 border border-transparent text-base font-medium rounded-lg shadow-lg text-white bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 focus:ring-purple-500 transition-all duration-300 transform hover:scale-105">Simpan Modul</button>
            </div>
        </form>
    </div>
</body>
